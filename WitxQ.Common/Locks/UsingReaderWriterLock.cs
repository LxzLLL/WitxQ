...
namespace WitxQ.Common.Locks
{
    /// <summary> 使用using代替lock操作的对象,可指定写入和读取锁定模式
    /// 方法:
    /// Read()     进入读取锁定模式  using (_Lock.Write()){...}
    /// Write()    进入写入锁定模式  using (_Lock.Read()){...}
    /// 另外我还加入2个额外的属性
    ...
    /// using (_Lock.Write()){}
    /// </summary>
    public class UsingReaderWriterLock<T>
    {
        #region 内部类
        ...
        public UsingReaderWriterLock(T data)
        {
            Enabled = true;
            _Data = data;
        }
